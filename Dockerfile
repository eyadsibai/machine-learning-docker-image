...
RUN curl -sL https://deb.nodesource.com/setup_4.x | sudo bash - && apt-get install -y --no-install-recommends nodejs \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/*
...
USER root
RUN apt-get remove -y git nodejs curl build-essential \
